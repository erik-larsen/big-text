...
|---|---|---|
| 0 sampled bars | under 1 | the file background with a trace of the hue, and every step-th line (file-relative index `j` with `j mod step == 0`) as a grey bar exactly 1 device pixel tall from indent to len, so the density stays about one bar per pixel row: the circuit-board texture of the whole map |
| 1 line bars | 1 to 3 | one grey bar per line from indent to len |
