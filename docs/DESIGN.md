...
|---|---|---|
| 0 sampled bars | under 1 | the file background, and every step-th line (file-relative index `j` with `j mod step == 0`) as a grey bar exactly 1 device pixel tall from indent to len, so the density stays about one bar per pixel row: the circuit-board texture of the whole map |
