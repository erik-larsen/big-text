...
*The same viewer on War and Peace, Dobbie's example, in his colours: white pages on the blue-grey ground of his demo, 1,401 pages and 2.63 million glyphs as one grid in reading order, tilted, the near pages at the text rung and the far rows at bars in the same frame.*
...
3. `atlas_viewer.py` opens the book fitted to the window. Everything in Controls works: `/` filters (`Natasha` finds 1,213 mentions on 299 pages), Enter flies to the next hit, `3` tilts the sheet (a book has no heights in 3D), hover names the part, chapter and page under the cursor. There is no tint, since a book has no commits.
