...
2. `atlas_layout.py` sees `"corpus": "book"` in the index and lays the pages out in reading order instead of a treemap: every part a full-width band of whole page rows, every page one cell, 65 pages across, and writes `layout.npz`: lines is the book's one metric, so in 3D every full page stands at the same height and only a chapter's short last page dips. Add `--preview docs/shots/war-and-peace_layout.png` to render the result with Pillow without opening a window.
