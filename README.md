...
| The ladder | [makepad](https://github.com/makepad/makepad) Studio code atlas (Rik Arends, 2026) | MIT (the repository's LICENSE, Makepad B.V.); its crates declare MIT OR Apache-2.0 | Semantic level of detail: one ladder from file tile to kind bands to line bars to tokens to text, chosen by pixels per line; a retained GPU working set with a device-derived budget | not pinned; `notes/makepad-code-atlas.md` records what the video and the public history say, and the public tree is the scale example under Run |
