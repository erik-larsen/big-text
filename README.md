...
3. `atlas_viewer.py` opens the book fitted to the window, its text in Literata at every size, glyphs placed by their own advances, and the hints and panel in the monospace face. Everything in Controls works: `/` filters (`Natásha` finds 1,213 mentions on 310 pages), Enter flies to the next hit, `3` tilts the sheet (a book has no heights in 3D), hover names the part, chapter and page under the cursor. There is no tint, since a book has no commits.
...
The same images for War and Peace, in `docs/shots/war-and-peace/`. The map fitted: one grid of 1,468 pages in reading order, 58 across, each page a white cell of one size in a grey gutter, the text the pale grey of its averaged ink.
...
The filter `Natásha`: 1,213 mentions on 310 pages, every page with one outlined in yellow, and the fly-to landing on the first, the hit filled yellow at text zoom.
...
- **Index.** `atlas_index.py` walks the tree (honouring `.gitignore`, skipping binaries and submodules), expands tabs, maps every character to one column, and runs a small regex tokenizer per language family (Rust, C-like, Python, shell, plain) that gives every character one of ten kinds. Items (functions, structs, enums, impls, modules) come from regexes with brace or indentation matching. The output is a handful of numpy arrays: characters, kinds, line offsets, file ranges, item ranges, and the tint. `book_index.py` writes the same index from a Project Gutenberg text: a heading line such as `BOOK ONE: 1805` opens a part, `CHAPTER I` a chapter, and every 40 lines of a chapter are a page, the file, named `Book One: 1805/Chapter I/p. 17`; the prose is reflowed to the page's line width in the book's face and kept in Windows-1252 (curly quotes, dashes, the ellipsis and accented letters as they are, anything outside it `?`; `index.json` names the encoding), and words, accented or not, are the identifier kind, with no string literals and no items.
- **The scheme.** An index may carry a `scheme`: the ground, the page (the file fill), the bar colour, one colour for every directory band in place of the hues, the ten kind colours, the five item colours, the ink of the bars and blocks, a face under `fonts/` with its leading, and whether lines are set flush; the viewer takes what is given over the code atlas's defaults and hands the colours to the shaders as uniforms. The book's is Dobbie's demo: pages as white quads on a `#a0a9af` ground, text black, the bands in the ground's own colour so the parts draw nothing, Literata for the face, and an ink measured from the face at index time, the mean coverage of a glyph's box over the book's own letter frequencies (15.6 percent for Literata, bars 17 percent and blocks 22), so nothing steps at a cut and a page seen from afar reads as the pale grey of averaged text, as his do. A scheme that names no ink, code's, gets the same measurement from the viewer at load: the token blocks carry the text's ink, and the bars keep the bright far view of the die shot and ease down to the blocks by 3 pixels per line. The scheme's `ink` is the bars' alpha far and near and the word blocks'. His own face was Centaur, read from the embedded fonts of the demo's source PDF; Centaur is Monotype's, and Literata is the book serif made for reading on screens under the OFL.
